/* 
 * Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.
 */
package com.newline53.sdk.models.operations;

import com.fasterxml.jackson.annotation.JsonCreator;
import com.fasterxml.jackson.annotation.JsonInclude.Include;
import com.fasterxml.jackson.annotation.JsonInclude;
import com.fasterxml.jackson.annotation.JsonProperty;
import com.newline53.sdk.utils.Utils;
import jakarta.annotation.Nonnull;
import jakarta.annotation.Nullable;
import java.lang.Override;
import java.lang.String;
import java.util.Optional;
import org.openapitools.jackson.nullable.JsonNullable;


public class CreateTransferWireTransmitterUnstructuredAddress {
    /**
     * Up to 35 characters. Cannot contain \# @ $! " % &amp; * ; &lt; &gt; { } [ ] _ ^ \ ~
     */
    @JsonInclude(Include.NON_ABSENT)
    @JsonProperty("line1")
    private JsonNullable<String> line1;

    /**
     * Optional 35 characters. Cannot contain \# @ $! " % &amp; * ; &lt; &gt; { } [ ] _ ^ \ ~
     */
    @JsonInclude(Include.NON_ABSENT)
    @JsonProperty("line2")
    private JsonNullable<String> line2;

    /**
     * Optional 32 characters. Note that this length is shorter than the other lines. Cannot contain \# @
     * $!
     * 
     * <p>" % &amp; * ; &lt; &gt; { } [ ] _ ^ \ ~
     */
    @JsonInclude(Include.NON_ABSENT)
    @JsonProperty("line3")
    private JsonNullable<String> line3;


    @JsonInclude(Include.NON_ABSENT)
    @JsonProperty("country")
    private String country;

    /**
     * Name of the Transmitter.
     */
    @JsonProperty("name")
    private String name;

    /**
     * Up to 24 digits, supplied by Transmitter. Numeric only.
     */
    @JsonProperty("transmitter_identifier")
    private String transmitterIdentifier;

    @JsonCreator
    public CreateTransferWireTransmitterUnstructuredAddress(
            @JsonProperty("line1") @Nullable JsonNullable<String> line1,
            @JsonProperty("line2") @Nullable JsonNullable<String> line2,
            @JsonProperty("line3") @Nullable JsonNullable<String> line3,
            @JsonProperty("country") @Nullable String country,
            @JsonProperty("name") @Nonnull String name,
            @JsonProperty("transmitter_identifier") @Nonnull String transmitterIdentifier) {
        this.line1 = Optional.ofNullable(line1)
            .orElse(JsonNullable.undefined());
        this.line2 = Optional.ofNullable(line2)
            .orElse(JsonNullable.undefined());
        this.line3 = Optional.ofNullable(line3)
            .orElse(JsonNullable.undefined());
        this.country = country;
        this.name = Optional.ofNullable(name)
            .orElseThrow(() -> new IllegalArgumentException("name cannot be null"));
        this.transmitterIdentifier = Optional.ofNullable(transmitterIdentifier)
            .orElseThrow(() -> new IllegalArgumentException("transmitterIdentifier cannot be null"));
    }
    
    public CreateTransferWireTransmitterUnstructuredAddress(
            @Nonnull String name,
            @Nonnull String transmitterIdentifier) {
        this(null, null, null,
            null, name, transmitterIdentifier);
    }

    /**
     * Up to 35 characters. Cannot contain \# @ $! " % &amp; * ; &lt; &gt; { } [ ] _ ^ \ ~
     */
    public JsonNullable<String> line1() {
        return this.line1;
    }

    /**
     * Optional 35 characters. Cannot contain \# @ $! " % &amp; * ; &lt; &gt; { } [ ] _ ^ \ ~
     */
    public JsonNullable<String> line2() {
        return this.line2;
    }

    /**
     * Optional 32 characters. Note that this length is shorter than the other lines. Cannot contain \# @
     * $!
     * 
     * <p>" % &amp; * ; &lt; &gt; { } [ ] _ ^ \ ~
     */
    public JsonNullable<String> line3() {
        return this.line3;
    }

    public Optional<String> country() {
        return Optional.ofNullable(this.country);
    }

    /**
     * Name of the Transmitter.
     */
    public String name() {
        return this.name;
    }

    /**
     * Up to 24 digits, supplied by Transmitter. Numeric only.
     */
    public String transmitterIdentifier() {
        return this.transmitterIdentifier;
    }

    public static Builder builder() {
        return new Builder();
    }


    /**
     * Up to 35 characters. Cannot contain \# @ $! " % &amp; * ; &lt; &gt; { } [ ] _ ^ \ ~
     */
    public CreateTransferWireTransmitterUnstructuredAddress withLine1(@Nullable String line1) {
        this.line1 = JsonNullable.of(line1);
        return this;
    }


    /**
     * Optional 35 characters. Cannot contain \# @ $! " % &amp; * ; &lt; &gt; { } [ ] _ ^ \ ~
     */
    public CreateTransferWireTransmitterUnstructuredAddress withLine2(@Nullable String line2) {
        this.line2 = JsonNullable.of(line2);
        return this;
    }


    /**
     * Optional 32 characters. Note that this length is shorter than the other lines. Cannot contain \# @
     * $!
     * 
     * <p>" % &amp; * ; &lt; &gt; { } [ ] _ ^ \ ~
     */
    public CreateTransferWireTransmitterUnstructuredAddress withLine3(@Nullable String line3) {
        this.line3 = JsonNullable.of(line3);
        return this;
    }


    public CreateTransferWireTransmitterUnstructuredAddress withCountry(@Nullable String country) {
        this.country = country;
        return this;
    }


    /**
     * Name of the Transmitter.
     */
    public CreateTransferWireTransmitterUnstructuredAddress withName(@Nonnull String name) {
        this.name = Utils.checkNotNull(name, "name");
        return this;
    }


    /**
     * Up to 24 digits, supplied by Transmitter. Numeric only.
     */
    public CreateTransferWireTransmitterUnstructuredAddress withTransmitterIdentifier(@Nonnull String transmitterIdentifier) {
        this.transmitterIdentifier = Utils.checkNotNull(transmitterIdentifier, "transmitterIdentifier");
        return this;
    }


    @Override
    public boolean equals(java.lang.Object o) {
        if (this == o) {
            return true;
        }
        if (o == null || getClass() != o.getClass()) {
            return false;
        }
        CreateTransferWireTransmitterUnstructuredAddress other = (CreateTransferWireTransmitterUnstructuredAddress) o;
        return 
            Utils.enhancedDeepEquals(this.line1, other.line1) &&
            Utils.enhancedDeepEquals(this.line2, other.line2) &&
            Utils.enhancedDeepEquals(this.line3, other.line3) &&
            Utils.enhancedDeepEquals(this.country, other.country) &&
            Utils.enhancedDeepEquals(this.name, other.name) &&
            Utils.enhancedDeepEquals(this.transmitterIdentifier, other.transmitterIdentifier);
    }
    
    @Override
    public int hashCode() {
        return Utils.enhancedHash(
            line1, line2, line3,
            country, name, transmitterIdentifier);
    }
    
    @Override
    public String toString() {
        return Utils.toString(CreateTransferWireTransmitterUnstructuredAddress.class,
                "line1", line1,
                "line2", line2,
                "line3", line3,
                "country", country,
                "name", name,
                "transmitterIdentifier", transmitterIdentifier);
    }

    @SuppressWarnings("UnusedReturnValue")
    public final static class Builder {

        private JsonNullable<String> line1;

        private JsonNullable<String> line2;

        private JsonNullable<String> line3;

        private String country;

        private String name;

        private String transmitterIdentifier;

        private Builder() {
          // force use of static builder() method
        }

        /**
         * Up to 35 characters. Cannot contain \# @ $! " % &amp; * ; &lt; &gt; { } [ ] _ ^ \ ~
         */
        public Builder line1(@Nullable String line1) {
            this.line1 = JsonNullable.of(line1);
            return this;
        }

        /**
         * Optional 35 characters. Cannot contain \# @ $! " % &amp; * ; &lt; &gt; { } [ ] _ ^ \ ~
         */
        public Builder line2(@Nullable String line2) {
            this.line2 = JsonNullable.of(line2);
            return this;
        }

        /**
         * Optional 32 characters. Note that this length is shorter than the other lines. Cannot contain \# @
         * $!
         * 
         * <p>" % &amp; * ; &lt; &gt; { } [ ] _ ^ \ ~
         */
        public Builder line3(@Nullable String line3) {
            this.line3 = JsonNullable.of(line3);
            return this;
        }

        public Builder country(@Nullable String country) {
            this.country = country;
            return this;
        }

        /**
         * Name of the Transmitter.
         */
        public Builder name(@Nonnull String name) {
            this.name = Utils.checkNotNull(name, "name");
            return this;
        }

        /**
         * Up to 24 digits, supplied by Transmitter. Numeric only.
         */
        public Builder transmitterIdentifier(@Nonnull String transmitterIdentifier) {
            this.transmitterIdentifier = Utils.checkNotNull(transmitterIdentifier, "transmitterIdentifier");
            return this;
        }

        public CreateTransferWireTransmitterUnstructuredAddress build() {
            return new CreateTransferWireTransmitterUnstructuredAddress(
                line1, line2, line3,
                country, name, transmitterIdentifier);
        }

    }
}
